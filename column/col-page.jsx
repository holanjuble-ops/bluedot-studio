          ...
          <Reveal delay={120}>
            <div style={{ borderTop: '1px solid #DDE3FF' }}>
              {STARTER_PAGE.map((s, i) => (
                <a key={s.no} href={ARTICLE_PATHS[s.id]} className="link-ul" style={{ display: 'flex', alignItems: 'center', gap: isMobile ? 14 : 24, padding: `${isMobile ? 20 : 26}px 4px`, borderBottom: '1px solid #DDE3FF', textDecoration: 'none' }}>
          ...
          <div style={{ marginTop: isMobile ? 24 : 28, borderTop: '1px solid var(--bd-line)' }}>
            {filteredCols.map((c, i) => (
              <Reveal key={c.id} delay={Math.min(i, 4) * 60}>
                {isMobile ? (
                  <a href={ARTICLE_PATHS[c.id]} className="link-ul" style={{ display: 'block', padding: '26px 0', borderBottom: '1px solid var(--bd-line)', textDecoration: 'none', color: 'inherit' }}>
                    <div>
                  ...
                  <a href={ARTICLE_PATHS[c.id]} className="link-ul" style={{ display: 'grid', gridTemplateColumns: '72px 1fr', gap: 28, alignItems: 'center', padding: '30px 0', borderBottom: '1px solid var(--bd-line)', textDecoration: 'none', color: 'inherit' }}>
